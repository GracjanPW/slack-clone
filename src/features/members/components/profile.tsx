...
export const Profile = ({ memberId, onClose }: ProfileProps) => {
  ...
  const [RoleDialog, confirmRole] = useConfirm(
    "Change role",
  ...
  const { mutate: updateMember, isPending: isPendingUpdate } =
    useUpdateMember();
  const { mutate: removeMember, isPending: isPendingRemove } =
    useRemoveMember();

  const isPending = isPendingRemove || isPendingUpdate;
  ...
  const avatrFallback = member?.user.name?.charAt(0).toUpperCase();
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              ...
              <Link
                ...
                className="text-sm hover:underline text-[#2476b4]"
              >
                {member.user.email}
              </Link>
            </div>
